...
RECURSIVE SUBROUTINE CALCULATE_AZ
  USE GEOMC; USE GLOBAL; USE TRANS; USE PREC; USE EDDY; USE KINETIC; use macrophytec; USE LOGICC; USE MAIN, ONLY:WARNING_OPEN;SAVE

  REAL, DIMENSION (2) :: SIG
  REAL                :: TKEMIN1,TKEMIN2,RIAZ0,AZ0,RIAZ1,EXPAZ,DEPTHL,DEPTHR,ZDLR,DEPTH,USTAR,ZD
  REAL                :: SLM,VISCK,VISCF,HRAD,EFFRIC,GC2,RI,BUOY,USTARBKT,WALLFUNCTION,UDR,UDL,CONVKX,CONVEX,CONVKZ,CONVEZ,ATA,AB
  ...
  ELSE
    ...
    DO K=KT,KBMIN(I)-1
      ...
      RIAZ0   =  LOG(AZ0/AZMAX(JW))*0.666666667                                                  ! /1.5
...
!CALCULATE HORIZONTAL PORTION OF THE SPLIT K-E EQUATIONS FOR INTERIOR LAYERS
  DO K = KT+1,KB(I)-1		
	IF (MANNINGS_N(JW)) THEN
	   HRAD = BH1(KT,I)/(B(KTI(I),I)-B(KT+1,I)+2.*AVH1(KT,I))                                !HRAD = BHR(K,I)/(BR(K,I)-BR(K+1,I)+2.0*H(K,JW))
	...
	BOUK = MAX(AZ(K,I)*G*(RHO(K+1,I)-RHO(K,I))/(H(K,JW)*RHOW),0.0)
	PRDK = AZ(K,I)*(0.5*(U(K,I)+U(K,I-1)-U(K+1,I)-U(K+1,I-1))/(H(K,JW)*0.5 + H(K+1,JW)*0.5))**2.0                 !/2.0  /2.0
	IF(.NOT.TKELATPRD(JW))THEN
	  PRHE = 0.0
	  PRHK = 0.0
	ELSE
	  PRHE = TKELATPRDCONST(JW)*GC2**1.25*ABS(0.5*(U(K,I)+U(K,I-1)))**4.0/(0.5*B(K,I))**2.0
      PRHK = GC2/(0.5*B(K,I))*ABS(0.5*(U(K,I)+U(K,I-1)))**3.0
    ENDIF
	UNST	   = PRDK-TKE(K,I,2)
	UNSE	   = 1.44*TKE(K,I,2)/TKE(K,I,1)*PRDK-1.92*(TKE(K,I,2)/TKE(K,I,1)*TKE(K,I,2))
...
INTEGER :: K,MULTIPLIER
    ...
    IF(SEMILOG(UBOUND,ABS((U(KB(I),I)+U(KB(I),I-1))*.5),(H(KB(I),JW))/2.0,VISCK)>0) THEN               !FRIC(I),
	  ...
	  ENDDO
	ENDIF
	LBOUND = VISCK/(E(I)*(H(KB(I),JW))/2.0)+NONZERO
	MULTIPLIER = 1
	DO WHILE(SEMILOG(LBOUND,ABS((U(KB(I),I)+U(KB(I),I-1))*.5),(H(KB(I),JW))/2.0,VISCK)<0)
		...
		ENDIF
	ENDDO    	
    WALLFUNCTION = RTBIS(LBOUND,UBOUND,1E-15,ABS((U(KB(I),I)+U(KB(I),I-1))*.5),(H(KB(I),JW))/2.0,VISCK)           !,FRIC(I)
    IF(STRICKON(JW))THEN
	  V = VISCK
	  UST = WALLFUNCTION
	  IF (MANNINGS_N(JW)) THEN
	  ...
	  ELSE
        PERIMETER = B(KB(I),I)+2.0*H(KB(I),JW)
        DO K=KT,KB(I)-1
        ...
        DO K=KT,KB(I)
          AREA = AREA + BH1(K,I)
        END DO
	    KS = ((STRICK(JW)*1.0/FRIC(I)*(AREA/PERIMETER)**(0.16666666667))**6)        !1.0/6.0
	  ENDIF
...
USE EDDY;USE GLOBAL;
REAL, INTENT(IN) :: UST,URES,Y,V
REAL :: SEMILOG
  SEMILOG = 0.41*URES/LOG(E(I)*Y*UST/V) - UST
END FUNCTION SEMILOG

FUNCTION RTBIS(X1,X2,XACC,U,Y,V)
USE GLOBAL, ONLY:WRN,JW; USE EDDY, ONLY:AZC; USE SCREENC, ONLY:JDAY
IMPLICIT NONE
REAL, INTENT(IN) :: X1,X2,XACC,U,Y,V
REAL :: RTBIS,SEMILOG
EXTERNAL SEMILOG
...
REAL    :: DEPTHL,DEPTHR,DEPTH,PERIMETER,AREA,UAVG
INTEGER :: K
  DEPTHL = (ELWS(I)-EL(KB(I),I)    +H2(KB(I),I)  *COSA(JB))/COSA(JB)                                              !EL(KT,I)  -Z(I)  *COSA(JB)
  DEPTHR = (ELWS(I+1)-EL(KB(I+1),I+1)+H2(KB(I+1),I)*COSA(JB))/COSA(JB)                                                     !EL(KT,I+1)-Z(I+1)*COSA(JB)
  DEPTH = (DEPTHR+DEPTHL)*0.5
