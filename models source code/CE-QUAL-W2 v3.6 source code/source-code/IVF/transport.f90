...
! Ultimate multipliers
  ...
  IF (ULTIMATE(JW)) THEN
    DO JB=BS(JW),BE(JW)
      FORALL(I=US(JB):DS(JB))                                                         !DO I=US(JB),DS(JB)
    ...
    DO I=IU,ID-1
      ...
      DO K=KT,KB(I)
        ...
        IF (U(K,I) >= 0.0) THEN
          ...
          ALFA(K,I)      =  2.0*(DX(K,I)*DLT/(SF1X(K,I)*SF1X(K,I))-(1.0-COUR*COUR)*0.1666667)*SF3X(K,I,1)     !/6.0
          AD1X(K,I) = (ALFA(K,I)-COUR*SF8X(K,I,1)*0.5)/SF5X(K,I,1)
          AD2X(K,I) =  SF4X(K,I,1)+(ALFA(K,I)-COUR*SF9X(K,I,1) *0.5)/SF6X(K,I,1)
          AD3X(K,I) =  SF2X(K,I,1)+(ALFA(K,I)-COUR*SF10X(K,I,1)*0.5)/SF7X(K,I,1)
        ELSE
          RATS(K,I)      =  RATD(I+1)
          CURS1(K,I)     =  CURX1(I+1)
          CURS2(K,I)     =  CURX2(I+1)
          ...
          ALFA(K,I)      =  2.0*(DX(K,I)*DLT/(SF1X(K,I)*SF1X(K,I))-(1.0-COUR*COUR)*0.1666667)*SF3X(K,I,2)    !/6.0
          AD1X(K,I) =  SF2X(K,I,2)+(ALFA(K,I)-COUR*SF8X(K,I,2)*0.5)/SF5X(K,I,2)
          AD2X(K,I) =  SF4X(K,I,2)+(ALFA(K,I)-COUR*SF9X(K,I,2)*0.5)/SF6X(K,I,2)
          AD3X(K,I) = (ALFA(K,I)-COUR*SF10X(K,I,2)*0.5)/SF7X(K,I,2)
        END IF
      END DO
    END DO
!$OMP END PARALLEL DO
      ...
      DO K=KT,KB(I)-1
        IF (W(K,I) >= 0.0) THEN
          ...
          IF (K <= KT+1) THEN
            ...
            CURS1Z(K,I) =  2.0*HM*HM/(AVH1(KT,I)+AVH1(K,I))/AVH1(KT,I)
            CURS2Z(K,I) = -2.0*HM*HM/(AVH1(KT,I)*AVH1(K,I))
            CURS3Z(K,I) =  2.0*HM*HM/(AVH1(KT,I)+AVH1(K,I))/AVH1(K,I)
            IF (K == KT) THEN
              HM    =  H1(KT,I)
              RATSZ(K,I)  =  1.0
              CURS3Z(K,I) =  1.0
          ...
          ALFAZ(K,I) =  2.0*(DZQ(K,I)*DLT/(SF1Z(K,JW)*SF1Z(K,JW))-(1.0-COUR*COUR)*0.16666667)*SF2Z(K,1,JW)     !/6.0
          AD1Z(K,I) = (ALFAZ(K,I)-COUR*SF8Z(K,1,JW)*0.5)/SF5Z(K,1,JW)
          AD2Z(K,I) =  SF4Z(K,1,JW)+(ALFAZ(K,I)-COUR*SF9Z(K,1,JW) *0.5)/SF6Z(K,1,JW)
          AD3Z(K,I) =  SF3Z(K,1,JW)+(ALFAZ(K,I)-COUR*SF10Z(K,1,JW)*0.5)/SF7Z(K,1,JW)
        ELSE
          ...
          END IF
          COUR      =  W(K,I)*DLT/SF1Z(K,JW)
          ALFAZ(K,I) =  2.0*(DZQ(K,I)*DLT/(SF1Z(K,JW)*SF1Z(K,JW))-(1.0-COUR*COUR)*0.16666667)*SF2Z(K,2,JW)    !/6.0
          AD1Z(K,I) =  SF3Z(K,2,JW)+(ALFAZ(K,I)-COUR*SF8Z(K,2,JW)*0.5)/SF5Z(K,2,JW)
          AD2Z(K,I) =  SF4Z(K,2,JW)+(ALFAZ(K,I)-COUR*SF9Z(K,2,JW)*0.5)/SF6Z(K,2,JW)
          AD3Z(K,I) = (ALFAZ(K,I)-COUR*SF10Z(K,2,JW)*0.5)/SF7Z(K,2,JW)
        END IF
      END DO
    END DO
!$END PARALLEL DO
