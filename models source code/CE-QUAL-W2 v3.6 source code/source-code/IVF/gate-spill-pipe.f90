...
SUBROUTINE GATE_FLOW
  USE STRUCTURES; USE GLOBAL; USE GEOMC
    INTEGER :: JG,ISUB,IGT
    REAL    :: ELIU,ELID,HTAIL,HENERGY,DLEL
      ...
      IF (ELID > EGT(JG) .OR. ELIU > EGT(JG)) THEN
        ...
        IF (IGT == 0) THEN
          IF (ISUB == 0) THEN
            ...
            IF (ELID > EGT(JG)) DLEL = ELIU-ELID                                       ! SW 6/7/10
            IF (DLEL < 0.0) THEN
              DLEL    = -DLEL
...
SUBROUTINE SPILLWAY_FLOW
  ...
  DO JS=1,NSP
    ...
    IF (ELID >= ESP(JS) .OR. ELIU >= ESP(JS)) THEN
      ...
      IF (ISUB == 0) THEN
        DLEL = ELIU-ESP(JS)
        IF (ELID > ESP(JS)) DLEL = ELIU-ELID                                   ! SW 6/7/10
        IF (DLEL < 0.0) THEN
          DLEL    = -DLEL
...
SUBROUTINE PIPE_FLOW_INITIALIZE
  ...
  REAL :: JDAY,DTQ,DLTX,EL1,EL2,HIE,EPS,DCHECK,D1,D2,DTEST,DCRIT,DEPTHCRIT,VTOT,TOTT
  INTEGER :: JP,NIT
...
SUBROUTINE OPEN_CHANNEL_INITIALIZE
  ...
  REAL    :: DLTX,PHI,VTOT,SLOPE,DIST,BEPR1,BEPR2,BC1,EL1,BC2,EL2,WLSLOPE,DLTX2,BAR1,BAREA,RAD1,RAD2
  REAL    :: TWIDTH,VAVG,QSUM,QAVG,QOUT,WETPER,BAR2
...
! Type declarations
  ...
  REAL                                 :: JDAY,DT,D
  REAL,    ALLOCATABLE, DIMENSION(:)   :: Y,   B,   V,   CAREA, TOPW,  BELEV, Q, VOLD, YOLD     ! CB 10/4/07
...
FUNCTION DEPTHCRIT (FLOW)
  USE STRUCTURES
  REAL :: DEPTHCRIT,FLOW,X1,X2,TOL,ZBRENT1
  X1        = DIA/1.0E7
  X2        = DIA
  TOL       = 0.001
...
FUNCTION CDFUNC (DEPTH,FLOW)
  USE STRUCTURES
  REAL  :: DEPTH,FLOW,CDFUNC,BAREA,TWIDTH
...
FUNCTION ZBRENT1 (X1,X2,TOL,BARG)
  ...
  INTEGER         :: I,J,ITER
  REAL            :: F1,F2,FUNC,X1,X2,TOL,BARG,BA,B,FA,FB,FC
  REAL            :: C,D,E,TOL1,XM,S,P,Q,R,ZBRENT1
