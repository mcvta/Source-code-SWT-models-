...
!** Reaeration factor
    ...
    ELSE IF (NEQN(JW) == 1) THEN                                                                           !Thomann and Fitzpatrick
      REAER(I) = (0.728*SQRT(WIND10(I))-0.317*WIND10(I)+0.0372*WIND10(I)**2)+3.93*SQRT(UAVG)/(ADEPTH)**0.5
    ELSE IF (NEQN(JW) == 2) THEN                                                   !User Defined
      REAER(I) = RCOEF1(JW)*(UAVG**RCOEF2(JW))*(ADEPTH**RCOEF3(JW))+(0.5+RCOEF4(JW)*WIND10(I)*WIND10(I))
    END IF
  END IF
