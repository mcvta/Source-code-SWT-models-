...
# Add filtered data
data_temp = data.loc[:,['Temp(In)','Tair','Tdew', 'Wind','HR']]

tmp2 = data_temp.rolling(31, center=False, axis=0, min_periods=1).mean()
tmp2.columns = ['TempinMeanFilter_31', 'TairMeanFilter_31', 'TdewMeanFilter_31','TWindMeanFilter_31','HRMeanFilter_31']

tmp3 = data_temp.rolling(31, center=False, axis=0, min_periods=1).std()
tmp3.columns = ['TempinStdFilter_31', 'TairStdFilter_31', 'TdewStdFilter_31','TWindStdFilter_31','HRStdFilter_31']

tmp4 = data_temp.rolling(31 * 24, center=False, axis=0, min_periods=1).mean()
tmp4.columns = ['TempinMeanFilter_31', 'TairMeanFilter_31', 'TdewMeanFilter_31','TWindMeanFilter_31','HRMeanFilter_31']

tmp5 = data_temp.rolling(31 * 24, center=False, axis=0, min_periods=1).std()
...
for _ in range(n_training_runs):
    ...
    # Note: it's important to exclude evaluation in order to accurately measure
    cgnet.train(XTrainScaled, yTrainScaled, epochs=5000)

    training_time = time.time() - start_time
    training_times.append(training_time)
    print(f"Training took {round(training_time, 2)} seconds")

    start_time = time.time()
    prediction = cgnet.predict(XTestScaled)
    prediction_time = time.time() - start_time
    prediction_times.append(prediction_time)
    print(f"Prediction took {round(prediction_time, 2)} seconds")
