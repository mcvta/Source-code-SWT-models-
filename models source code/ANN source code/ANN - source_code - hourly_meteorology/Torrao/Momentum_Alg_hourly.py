...
# Import data
data = pd.read_excel('InputTorrao.xlsx', index_col=0, header=0)
...
data['HourCos'] = np.cos(2 * np.pi * data.index.hour / 24)
data['HourSin'] = np.sin(2 * np.pi * data.index.hour / 24)
...
tmp4 = data.loc[:,['Temp(In)','Tair','Tdew', 'Wind','HR']].rolling(31 * 24, center=False, axis=0, min_periods=1).mean()
tmp4.columns = ['TempinMeanFilter_31', 'TairMeanFilter_31', 'TdewMeanFilter_31','TWindMeanFilter_31','HRMeanFilter_31']

tmp5 = data.loc[:,['Temp(In)','Tair','Tdew', 'Wind','HR']].rolling(31 * 24, center=False, axis=0, min_periods=1).std()
tmp5.columns = ['TempinStdFilter_31', 'TairStdFilter_31', 'TdewStdFilter_31','TWindStdFilter_31','HRStdFilter_31']

data = pd.concat((data, tmp2, tmp3 ** 2, tmp4, tmp5 ** 2), axis=1)
...
# Import data
dataR = pd.read_excel('RealTorrao.xlsx', index_col=0, header=0)
...
dataR['Hour'] = dataR.index.hour
dataR['HourCos'] = np.cos(2 * np.pi * dataR.index.hour / 24)
dataR['HourSin'] = np.sin(2 * np.pi * dataR.index.hour / 24)
...
tmp4 = dataR.loc[:,['Temp(In)','Tair','Tdew', 'Wind','HR']].rolling(31 * 24, center=False, axis=0, min_periods=1).mean()
tmp4.columns = ['TempinMeanFilter_31', 'TairMeanFilter_31', 'TdewMeanFilter_31','TWindMeanFilter_31','HRMeanFilter_31']

tmp5 = dataR.loc[:,['Temp(In)','Tair','Tdew', 'Wind','HR']].rolling(31 * 24, center=False, axis=0, min_periods=1).std()
tmp5.columns = ['TempinStdFilter_31', 'TairStdFilter_31', 'TdewStdFilter_31','TWindStdFilter_31','HRStdFilter_31']

dataR = pd.concat((dataR, tmp2, tmp3 ** 2, tmp4, tmp5 ** 2), axis=1)
